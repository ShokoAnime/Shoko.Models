...
public class MediaContainer : IMediaInfo
{
    ...
    public List<VideoStream> VideoStreams =>
        _videos ?? (_videos = media?.track?.Where(a => a?.type == StreamType.Video).Cast<VideoStream>().ToList() ?? new());

    [JsonIgnore]
    [IgnoreMember]
    public List<AudioStream> AudioStreams =>
        _audios ?? (_audios = media?.track?.Where(a => a?.type == StreamType.Audio).Cast<AudioStream>().ToList() ?? new());

    [JsonIgnore]
    [IgnoreMember]
    public List<TextStream> TextStreams =>
        _texts ?? (_texts = media?.track?.Where(a => a?.type == StreamType.Text).Cast<TextStream>().ToList() ?? new());
            ...
            if (menu?.extra != null)
            {
                foreach (var (key, value) in menu.extra)
                {
                    if (string.IsNullOrEmpty(key))
                        continue;
                    var parts = key.Substring(1).Split('_', 4);
                    var timestamp = TimeSpan.Parse($"{parts[0]}:{parts[1]}:{parts[2]}.{parts[3]}");
...
public abstract class Stream : IStream
{
    ...
    TextLanguage? IStream.Language => Language?.ToTextLanguage();
