...
namespace Shoko.Models.MediaInfo
{
    public static class MediaInfoUtils
    {
        ...
        private static readonly double[] Ratios = { FourThirds, SixteenNine, TwentyOneNine };
        ...
        private static readonly Dictionary<string, Tuple<string, string>> LanguageMapping_2_1_Name = new()
        {
        ...
        public static string GetStandardResolution(Tuple<int, int> res)
        {
            ...
            double nearest = FindClosestRatio(Ratios, ratio);
            ...
            switch (nearest)
            {
                ...
                case SixteenNine:
                {
                    ...
                    int key = FindClosestRatio(ResolutionArea169.Keys, area);
                    return ResolutionArea169[key];
                }
                case FourThirds:
                {
                    int area = res.Item1 * res.Item2;
                    int key = FindClosestRatio(ResolutionArea43.Keys, area);
                    return ResolutionArea43[key];
                }
                case TwentyOneNine:
                {
                    int area = res.Item1 * res.Item2;
                    int key = FindClosestRatio(ResolutionArea219.Keys, area);
        ...
        public static string TranslateCodec(Stream stream)
        {
            ...
            if (id != null && CodecIDs.ContainsKey(id))
                return CodecIDs[id].ToUpper();
            ...
            {
                if (!string.IsNullOrEmpty(id) && SubFormats.TryGetValue(id, out var codecFormat))
